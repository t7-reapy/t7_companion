...
- `plugin/skills/*/SKILL.md` — the plugin's skills. `setup` is the **agent's** install action (it runs `install/install.sh` + `claude mcp add`) — i.e. the agent-facing mirror of the README's install, not a separate install method. Domain skills are named `bo3-*`. **Adding, renaming, or removing one has two required follow-ups, not one:** update `bo3-knowledge`'s frontmatter `description` (it names every other `bo3-*` skill by name to route around trigger overlap) and `.claude-plugin/marketplace.json`'s plugin description. The README's own mermaid diagram deliberately does *not* enumerate skill names, specifically so it can't go stale the way it just did (fixed by de-listing rather than re-listing) — don't reintroduce a hardcoded skill list there.
